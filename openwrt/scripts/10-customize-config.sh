...
sed -i '/# timezone/i [ "$(cat /sys/class/dmi/id/sys_vendor 2>/dev/null)" = "Default string" ] \&\& grep -q "/tmp/sysinfo/model" /etc/rc.local || sed -i "/exit 0/i echo '\''Industrial Router'\'' > /tmp/sysinfo/model" /etc/rc.local\n' package/new/default-settings/default/zzz-default-settings
